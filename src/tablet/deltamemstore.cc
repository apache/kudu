...
void DeltaMemStore::Update(rowid_t row_idx,
...
rowid_t DeltaMemStore::DecodeKey(const Slice &key) {
  DCHECK_EQ(sizeof(rowid_t), key.size());
  return ntohl(*reinterpret_cast<const rowid_t *>(key.data()));
}
...
Status DMSIterator::PrepareBatch(size_t nrows) {
  ...
  while (iter_->IsValid()) {
    ...
    rowid_t row_idx = DeltaMemStore::DecodeKey(key);
...
Status DMSIterator::ApplyUpdates(size_t col_to_apply, ColumnBlock *dst) {
  ...
  while (!src.empty()) {
    ...
    // TODO: 64-bit rowids
    COMPILE_ASSERT(sizeof(rowid_t) == sizeof(uint32_t), err_non_32bit_rowid);
    rowid_t idx = DecodeFixed32(src.data());
