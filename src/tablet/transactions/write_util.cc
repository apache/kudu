...
Status CreatePreparedInsertsAndMutates(Tablet* tablet,
  ...
  tx_state->set_component_lock(component_lock_.Pass());
  ...
  if (ops_pb.rows().size() > 0) {
    ...
    if (!s.ok()) {
      ...
      tx_state->completion_callback()->set_error(s,
  ...
  BOOST_FOREACH(const DecodedRowOperation& op, decoded_ops) {
    ...
    switch (op.type) {
      case RowOperationsPB::INSERT:
      {
        ...
        RETURN_NOT_OK(tablet->CreatePreparedInsert(tx_state, row, &row_write));
        break;
      }
      ...
      case RowOperationsPB::DELETE:
      {
        ...
        RETURN_NOT_OK(tablet->CreatePreparedMutate(tx_state, row_key, mutation, &row_write));
        break;
      }
      default:
