...
TEST_F(PartialRowTest, ProtobufRoundtrip) {
  Schema schema = Schema(boost::assign::list_of(ColumnSchema("int8", INT8))
                                               (ColumnSchema("int16", INT16))
                                               (ColumnSchema("int32", INT32))
                                               (ColumnSchema("int64", INT64))
                                               (ColumnSchema("bool", BOOL))
                                               (ColumnSchema("float", FLOAT))
                                               (ColumnSchema("double", DOUBLE))
                                               (ColumnSchema("string", STRING))
                                               (ColumnSchema("binary", BINARY))
                                               (ColumnSchema("timestamp", TIMESTAMP))
                                               (ColumnSchema("null", STRING, true))
                                               (ColumnSchema("missing", STRING)),
                         1);

  KuduPartialRow row(&schema);
  ASSERT_OK(row.SetInt8("int8", 1));
  ASSERT_OK(row.SetInt16("int16", 2));
  ASSERT_OK(row.SetInt32("int32", 3));
  ASSERT_OK(row.SetInt64("int64", 4));
  ASSERT_OK(row.SetBool("bool", true));
  ASSERT_OK(row.SetFloat("float", 5.6));
  ASSERT_OK(row.SetDouble("double", 7.8));
  ...
  ASSERT_OK(row.SetNull("null"));

  PartialRowPB pb;
  ASSERT_OK(row.ToPB(&pb));
  KuduPartialRow row2(&schema);
  ASSERT_OK(KuduPartialRow::FromPB(pb, &row2));
  ...
  CHECK_EQ(2, int16_val);

  int32_t int32_val;
  ASSERT_OK(row2.GetInt32("int32", &int32_val));
  CHECK_EQ(3, int32_val);

  int64_t int64_val;
  ASSERT_OK(row2.GetInt64("int64", &int64_val));
  CHECK_EQ(4, int64_val);

  bool bool_val;
  ASSERT_OK(row2.GetBool("bool", &bool_val));
  CHECK_EQ(true, bool_val);

  float float_val;
  ASSERT_OK(row2.GetFloat("float", &float_val));
  ASSERT_FLOAT_EQ(5.6, float_val);

  double double_val;
  ASSERT_OK(row2.GetDouble("double", &double_val));
  ASSERT_FLOAT_EQ(7.8, double_val);
