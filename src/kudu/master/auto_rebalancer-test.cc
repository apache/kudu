...
class AutoRebalancerTest : public KuduTest {
  ...
  Status CreateAndStartCluster() {
    ...
    FLAGS_auto_leader_rebalancing_enabled = true; // Enable for testing.
  ...
  void CheckSomeMovesScheduled() {
    ...
    ASSERT_EVENTUALLY([&] {
      ASSERT_LT(leader_initial_loop_iterations,
                NumLoopIterations(leader_idx, BalanceThreadType::LEADER_REBALANCE));
      ASSERT_LE(0, NumMovesScheduled(leader_idx, BalanceThreadType::LEADER_REBALANCE));
    });
  }
