...
void RowSetMetadata::SetColumnDataBlocks(const map<ColumnId, BlockId>& blocks_by_col_id) {
...
void RowSetMetadata::CommitUpdate(const RowSetMetadataUpdate& update,
  ...
  removed->clear();
  {
    ...
    // Add new redo blocks.
    redo_delta_blocks_.reserve(redo_delta_blocks_.size() + update.new_redo_blocks_.size());
    ...
    for (const auto& b : undo_delta_blocks_) {
      if (const auto it = undos_to_remove.find(b); it == undos_to_remove.end()) {
        udb.emplace_back(b);
      } else {
