...
TabletServerOptions::TabletServerOptions() {
  rpc_opts.rpc_bind_addresses = FLAGS_tserver_rpc_bind_addresses;
  rpc_opts.num_acceptors_per_address = FLAGS_tserver_num_acceptors_per_address;
  rpc_opts.num_service_threads = FLAGS_tserver_num_service_threads;
  rpc_opts.default_port = TabletServer::kDefaultPort;

  webserver_opts.port = FLAGS_tserver_web_port;
  // The rest of the web server options are not overridable on a per-tablet-server
  // basis.

  wal_dir = FLAGS_tserver_wal_dir;
  data_dirs = strings::Split(FLAGS_tserver_data_dirs, ",",
                             strings::SkipEmpty());
