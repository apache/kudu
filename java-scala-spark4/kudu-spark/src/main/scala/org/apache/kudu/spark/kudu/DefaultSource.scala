...
class DefaultSource
  ...
  private def getOperationType(parameters: Map[String, String]): OperationType = {
    parameters
      .get(OPERATION)
      .map(stringToOperationType)
      .getOrElse(org.apache.kudu.spark.kudu.Upsert)
  }

  private def stringToOperationType(opParam: String): OperationType = {
    opParam.toLowerCase(Locale.ENGLISH) match {
      case "insert" => org.apache.kudu.spark.kudu.Insert
      case "insert_ignore" =>
        org.apache.kudu.spark.kudu.InsertIgnore
      case "insert-ignore" =>
        org.apache.kudu.spark.kudu.InsertIgnore
      case "upsert" => org.apache.kudu.spark.kudu.Upsert
      case "update" => org.apache.kudu.spark.kudu.Update
      case "update_ignore" =>
        org.apache.kudu.spark.kudu.UpdateIgnore
      case "update-ignore" =>
        org.apache.kudu.spark.kudu.UpdateIgnore
      case "delete" => org.apache.kudu.spark.kudu.Delete
      case "delete_ignore" =>
        org.apache.kudu.spark.kudu.DeleteIgnore
      case "delete-ignore" =>
        org.apache.kudu.spark.kudu.DeleteIgnore
      case _ =>
