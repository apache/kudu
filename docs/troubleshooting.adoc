...
Every Kudu daemon includes built-in tracing support based on the open source
link:https://www.chromium.org/developers/how-tos/trace-event-profiling-tool[Chromium Tracing]
framework. Tracing can be used to help diagnose latency issues or other problems
on Kudu servers.
...
WARNING: The tracing interface is known to work in recent versions of Google Chrome. Other browsers
may not work as expected.

==== Collecting a trace

After navigating to the tracing interface, click the 'Record' button on the top left corner
of the screen. When beginning to diagnose a problem, it is easiest to select all categories
using the provided button. Then hit 'Record' to begin recording a trace.

While the system is recording a trace, events are collected into an in-memory ring buffer.
This ring buffer is fixed size, so it will eventually fill up to 100%. However, new events
are still being collected while older events are being removed. While recording the trace,
trigger whatever behavior or workload you are interested in exploring.

After collecting for several seconds, hit the 'stop' button, which will download and
display the collected trace. Hitting the '?' key will bring up information on how to
use the tracing interface to explore the trace.

==== Saving a trace

Collected traces can be saved as JSON files for later analysis. Simply collect a trace
and hit the 'Save' button at the top of the screen. This results in a JSON document
which may be later loaded into the same interface using the 'Load' button.
...
These traces can give an indication of which part of the request was slow, and are
useful to include in bug reports related to RPC latency outliers.

=== Kernel Stack Watchdog Traces

Every Kudu server process has a background thread called the Stack Watchdog. This thread monitors the
other threads in the server for cases where they've blocked for longer-than-expected periods of time.
These traces can be indicative of operating-system level issues or heavily contended storage.
When the watchdog thread identifies a case of thread blockage, it logs an entry in the `WARNING`
log:
