...
. If building on RHEL 9.0 or newer, and it's necessary to build PostgreSQL
in `thirdparty` from source (e.g., if not using pre-built 3rd-party artifacts
or they aren't available for some reason), install the `perl-FindBin` package:
+
----
...
sudo yum -y install autoconf automake curl cyrus-sasl-devel cyrus-sasl-gssapi \
  ...
  krb5-server krb5-workstation krb5-devel libtool make openssl-devel patch \
  pkgconfig rsync unzip vim-common which
# If installing on RHEL/CentOS >= 9 and building 3rd-party PostgreSQL from
# source, uncomment the next line to install perl-FindBin module
#sudo yum -y install perl-FindBin
...
compiling Kudu. Some of the instructions below use link:https://brew.sh/[Homebrew]
...
[[build_cpp_client_only]]
== Building only Kudu {cpp} Client Library and its dependencies
For a use case where it's not necessary to build Kudu's server-side, but
only Kudu {cpp} client library, it's possible to save time and computing
resources by building a subset of third-party dependencies that are necessary
for building kudu_client library only.

. Build a subset of the third-party components that are necessary to build
Kudu {cpp} client library by specifying `client_only` command-line
argument for the `build-if-necessary.sh` script:
+
[source,bash]
----
$ build-support/enable_devtoolset.sh thirdparty/build-if-necessary.sh client_only
----

. Build Kudu client library. Choose a build directory for the intermediate
output, which can be anywhere in your filesystem except for the `kudu`
directory itself. Run `make`, explicitly specifying `kudu_client` and
`kudu_client_exported` targets.
+
[source,bash]
----
mkdir -p build/release
...
../../thirdparty/installed/common/bin/cmake \
  ...
  -DKUDU_CLIENT_ONLY=1 \
  ../..
make -j4 kudu_client kudu_client_exported
----
