...
from source. To run Kudu without installing anything, use the <<quickstart_vm, Cloudera
Kudu Quickstart VM>>.
// TODO Quickstart VM does not exist yet
...
durability levels.
- An EXT4 or XFS filesystem to store Kudu data and write-ahead logs (WALs).
...
. Click *Continue*. Kudu masters and tablet servers are started.
...
sudo yum install kudu-master                  # Service scripts for managing kudu-master
                                              # Use ONLY if you do not use Cloudera Manager

sudo yum install kudu-tserver                 # Service scripts for managing kudu-tserver
                                              # Use ONLY if you do not use Cloudera Manager
----
...
. Create a flagfile in `/etc/kudu/conf/gflagfile` with at least the following contents,
modified to match your environment. It should be world-readable.
+
----
# Minimum configuration for masters
...
----
// TODO probably change this to 'review and edit' later

. Adjust the Kudu default settings to use this flagfile. Edit `/etc/default/kudu`
and add the following argument to the list of arguments for `KUDU_MASTER_ARGS` and
`KUDU_TSERVER_ARGS`: `--flagfile=/etc/kudu/conf/gflagfile`
// TODO probably take this step out later
