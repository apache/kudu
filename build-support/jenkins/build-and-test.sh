...
scl enable devtoolset-3 thirdparty/build-if-necessary.sh
...
if [ "$BUILD_TYPE" = "ASAN" ]; then
  scl enable devtoolset-3 "CC=$CLANG CXX=$CLANG++ cmake -DKUDU_USE_ASAN=1 -DKUDU_USE_UBSAN=1 ."
  BUILD_TYPE=fastdebug
  BUILD_PYTHON=0
elif [ "$BUILD_TYPE" = "TSAN" ]; then
  scl enable devtoolset-3 "CC=$CLANG CXX=$CLANG++ cmake -DKUDU_USE_TSAN=1 ."
  BUILD_TYPE=fastdebug
  ...
  DO_COVERAGE=1
  BUILD_TYPE=debug
  scl enable devtoolset-3 "cmake -DKUDU_GENERATE_COVERAGE=1 ."
...
scl enable devtoolset-3 "cmake . -DCMAKE_BUILD_TYPE=${BUILD_TYPE}"
...
./thirdparty/installed/bin/ctest -j$NUM_PROCS $EXTRA_TEST_FLAGS || EXIT_STATUS=$?
