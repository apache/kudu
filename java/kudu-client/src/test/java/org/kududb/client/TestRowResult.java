...
public class TestRowResult extends BaseKuduTest {
  ...
  public void test() throws Exception {
    ...
    while (scanner.hasMoreRows()) {
      ...
      assertEquals((byte) 1, rr.getByte(allTypesSchema.getColumn(0).getName()));

      assertEquals((short) 2, rr.getShort(1));
      assertEquals((short) 2, rr.getShort(allTypesSchema.getColumn(1).getName()));

      assertEquals(3, rr.getInt(2));
      assertEquals(3, rr.getInt(allTypesSchema.getColumn(2).getName()));

      assertEquals(4, rr.getLong(3));
      assertEquals(4, rr.getLong(allTypesSchema.getColumn(3).getName()));

      assertEquals(true, rr.getBoolean(4));
      assertEquals(true, rr.getBoolean(allTypesSchema.getColumn(4).getName()));

      assertEquals(5.6f, rr.getFloat(5), .001f);
      assertEquals(5.6f, rr.getFloat(allTypesSchema.getColumn(5).getName()), .001f);

      assertEquals(7.8, rr.getDouble(6), .001);
      assertEquals(7.8, rr.getDouble(allTypesSchema.getColumn(6).getName()), .001f);

      assertEquals("string-value", rr.getString(7));
      assertEquals("string-value", rr.getString(allTypesSchema.getColumn(7).getName()));

      assertArrayEquals("binary-value".getBytes(), rr.getBinaryCopy(8));
      assertArrayEquals("binary-value".getBytes(),
          rr.getBinaryCopy(allTypesSchema.getColumn(8).getName()));

      ByteBuffer buffer = rr.getBinary(8);
      assertEquals(buffer, rr.getBinary(allTypesSchema.getColumn(8).getName()));
      ...
      assertEquals(true, rr.isNull(allTypesSchema.getColumn(9).getName()));

      assertEquals(10, rr.getLong(10));
      assertEquals(10, rr.getLong(allTypesSchema.getColumn(10).getName()));

      // We test with the column name once since it's the same method for all types, unlike above.
      assertEquals(Type.INT8, rr.getColumnType(allTypesSchema.getColumn(0).getName()));
