 ...
 * {@link ModernStrategy}, and {@link UnsupportedStrategy} strategies to
 * provide the legacy APIs as long as they are present and not degraded.
...
class CompositeStrategy
  ...
  static final CompositeStrategy INSTANCE =
  ...
  CompositeStrategy(ReflectiveStrategy.Loader loader) {
    ...
    try {
      ...
      try {
        ...
        log.debug(
            "Loaded legacy SecurityManager methods, will fall back to " +
                "modern methods after UnsupportedOperationException");
      } catch (NoSuchMethodException | ClassNotFoundException ex) {
        // This is expected for JRE <= 17
        log.debug("Unable to load modern Subject methods, relying only on legacy methods",
            ex);
      }
      ...
      try {
        ...
        log.debug(
            "Unable to load legacy SecurityManager methods, relying only on modern methods",
            e);
      } catch (NoSuchMethodException | ClassNotFoundException ex) {
    ...
    try {
      ...
      if (active != fallbackStrategy && fallbackStrategy != null) {
        if (activeStrategy.compareAndSet(active, fallbackStrategy)) {
          log.debug("Using fallback strategy after encountering degraded legacy method",
              e);
        }
        return action.apply(fallbackStrategy);
      }
      // If we're already using the fallback strategy, then there's nothing
      // to do to handle these exceptions.
      throw e;
    }
  }
